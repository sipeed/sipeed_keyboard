from mainwin import *
from PySide6.QtCore import (QByteArray, QDataStream, QFile, QFileInfo,
                          QIODevice, QPoint, QPointF, QRectF, Qt, QTimer)
from PySide6.QtWidgets import QApplication, QMainWindow, QFileDialog,QGraphicsScene,QGraphicsPixmapItem,QGraphicsItem,QStyle,QGraphicsTextItem,QMenu,QMessageBox
from PySide6.QtGui import QImage,QPen,QBrush,QTransform,QColor
...
class My_Application(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.ui.previous_mode.clicked.connect(self.prevMode)
        self.ui.next_mode.clicked.connect(self.nextMode)
...
'''
    ...
    def showAbout(self):
        MessageBox.information(class_instance, "About", "SMK Manager 1.0\n by Chrithon & Tinylib")
    ...
    def connectfunc(self):
        ...
        if(self.kb.wait_for_kb(1000)==False):
            MessageBox.critical(class_instance, "Error!", "Connection failed!")
    ...
    def nextMode(self):
        mode_id_num = self.ui.mode_id.intValue()
        if(mode_id_num < 3):
            mode_id_num += 1
        self.ui.mode_id.display(mode_id_num)
    ...
    def set_mode_func(self):
        if self.kb.isdeviceoen==False:
            return
        try:
            mode=int(self.ui.mode_id.value())
        except:
            mode=0
